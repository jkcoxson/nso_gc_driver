...
struct DSUStatus: Equatable {
    var isListening = false
    var port: UInt16 = 26760
    var clientCount = 0
...
final class DSUServer: @unchecked Sendable {
    ...
    private static let connectionIdleTimeout: TimeInterval = 5

    private let queue = DispatchQueue(label: "com.nso-gc-driver.dsu")
    private var listener: NWListener?
    private var activeConnections: [ObjectIdentifier: NWConnection] = [:]
    private var connectionActivity: [ObjectIdentifier: Date] = [:]
    ...
    var onStatus: ((DSUStatus) -> Void)?
    private var status = DSUStatus()
    private var packetCountThisSecond = 0
    private var lastRateUpdate = Date()
    private var statusTickScheduled = false
    ...
    private func startListener(startingAt firstCandidate: Int) -> Bool {
        ...
        for candidate in firstCandidate...Self.allowedPorts.upperBound {
            ...
            do {
                ...
                // Do not set NWListener.newConnectionLimit for UDP. Peers cannot signal
                // closure, so Network.framework would eventually fill the limit with
                // stale endpoints and stop delivering new discovery packets. The
                // server enforces its own bounded, expiring connection set below.
                ...
                newListener.stateUpdateHandler = { [weak self, weak newListener] state in
                    ...
                    case .ready:
                        ...
                        self.status.isListening = true
                        self.status.port = UInt16(candidate)
                        self.status.error = nil
                        self.publishStatus(force: true)
                        self.scheduleStatusTick()
                    case .failed(let error):
                        self.onLog?(
                            "DSU port \(candidate) is unavailable (\(error)); trying the next port")
                        self.status.isListening = false
    ...
    func stop() {
        queue.async { [weak self] in
            ...
            self.connectionActivity.removeAll()
            ...
            self.status.isListening = false
            self.status.clientCount = 0
            self.status.packetsPerSecond = 0
            self.publishStatus(force: true)
    ...
    private func receive(_ connection: NWConnection) {
        if activeConnections.count >= Self.maximumConnections,
            let oldestID = connectionActivity.min(by: { $0.value < $1.value })?.key,
            let oldestConnection = activeConnections[oldestID]
        {
            remove(oldestConnection)
            oldestConnection.cancel()
        }
        let connectionID = ObjectIdentifier(connection)
        activeConnections[connectionID] = connection
        connectionActivity[connectionID] = Date()
        status.clientCount = activeConnections.count
    ...
    private func remove(key: String, connection: NWConnection? = nil) {
        ...
        if let connection {
            ...
            connectionActivity.removeValue(forKey: connectionID)
        }
    ...
    private func notePacketActivity() {
        packetCountThisSecond += 1
    ...
    private func publishStatus(force: Bool) {
        ...
        if elapsed >= 1 {
            status.packetsPerSecond = Int((Double(packetCountThisSecond) / elapsed).rounded())
    ...
    private func scheduleStatusTick() {
        guard !statusTickScheduled else { return }
        statusTickScheduled = true
        queue.asyncAfter(deadline: .now() + 1) { [weak self] in
            guard let self else { return }
            self.statusTickScheduled = false
            guard self.listener != nil else { return }
            let cutoff = Date().addingTimeInterval(-Self.connectionIdleTimeout)
            let idleConnections = self.connectionActivity.compactMap { id, activity in
                activity < cutoff ? self.activeConnections[id] : nil
            }
    ...
    private func sendNextPadData(for key: PadSendKey) {
        ...
        pending.connection.send(
            ...
            completion: .contentProcessed {
                [weak self, weak connection = pending.connection] error in
                guard let self else { return }
